...
#define	MAX_BUCK_DUTY1  (uint16_t)(0.70 * HRTIMD_Period)    //MIX state buck最大占空比80%
#define MAX_BOOST_DUTY  (uint16_t)(0.70 * HRTIMA_Period)    //boost最大占空比40%
...
struct _FLAG
{
    ...
    uint16_t BBFlag;         //运行模式标志位，Buck模式，MIX混合模式
    uint16_t DRFlag;         //current direction flag,charge,discharge
    uint16_t Cloop;          //loop control mode
    uint16_t PWMENFlag;      //启动标志位
    uint16_t BBModeChange;   //工作模式切换标志位
